    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <!-- Client Select -->
                <div>
                    <label for="clientId" class="block text-sm font-medium text-gray-900 dark:text-white">Client *</label>
                ...
                <!-- Issue Date -->
                <div>
                    <label for="issueDate" class="block text-sm font-medium text-gray-900 dark:text-white">Issue Date *</label>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-4">
            <div class="flex items-center justify-between mb-4">
                ...
                <button type="button" wire:click="addLineItem" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition">
                    + Add Line Item
                </button>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-100 dark:bg-gray-700 border-b dark:border-gray-600">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-center font-semibold text-gray-900 dark:text-white">Quantity</th>
                            <th class="px-4 py-2 text-right font-semibold text-gray-900 dark:text-white">Unit Price</th>
                            <th class="px-4 py-2 text-left font-semibold text-gray-900 dark:text-white">Tax Rate</th>
                            <th class="px-4 py-2 text-right font-semibold text-gray-900 dark:text-white">Line Total</th>
                            <th class="px-4 py-2 text-right font-semibold text-gray-900 dark:text-white">Tax</th>
                            <th class="px-4 py-2 text-center font-semibold text-gray-900 dark:text-white">Action</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y dark:divide-gray-700">
                        @forelse($lineItems as $index => $item)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                ...
                                <!-- Quantity -->
                                <td class="px-4 py-2">
                                    <input type="number" step="0.01" min="0.01" wire:model.live.debounce.500ms="lineItems.{{ $index }}.quantity"
                                ...
                                <td class="px-4 py-2">
                                    <div class="flex items-center justify-end">
                                        ...
                                        <input type="number" step="0.01" min="0.01" wire:model.live.debounce.500ms="lineItems.{{ $index }}.unit_price"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div class="space-y-3 max-w-sm ml-auto">
                <div class="flex justify-between">
                    ...
                    <span class="font-semibold text-gray-900 dark:text-white">${{ number_format($subtotal / 100, 2) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-700 dark:text-gray-300">Tax Total:</span>
                    <span class="font-semibold text-gray-900 dark:text-white">${{ number_format($taxTotal / 100, 2) }}</span>
                </div>
                <div class="flex justify-between text-lg border-t dark:border-gray-700 pt-3">
                    ...
                    <span class="font-bold text-blue-600 dark:text-blue-400">${{ number_format($grandTotal / 100, 2) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-4">
            ...
            <a href="{{ route('invoices.index') }}" class="px-6 py-2 bg-gray-300 text-gray-900 rounded-lg hover:bg-gray-400 transition dark:bg-gray-600 dark:text-white">
                Cancel
            </a>
        </div>
    </form>

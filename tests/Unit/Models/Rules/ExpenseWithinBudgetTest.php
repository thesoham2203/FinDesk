...
test('expense with non-existent department fails validation', function (): void {
    $rule = new ExpenseWithinBudget(departmentId: 9999, amount: 50000); // Non-existent department
    ...
    expect($fails)->not->toBeEmpty();
    expect($fails[0])->toContain('Department not found');
});

test('expense with zero amount passes validation', function (): void {
    $department = Department::factory()->create(['monthly_budget' => 100000]); // 1000.00
    $rule = new ExpenseWithinBudget(departmentId: $department->id, amount: 0); // 0.00

    $fails = [];
    $rule->validate('amount', 0, function ($message) use (&$fails): void {
        $fails[] = $message;
    });
...
test('expense with negative amount fails validation', function (): void {
    $department = Department::factory()->create(['monthly_budget' => 100000]); // 1000.00
    $rule = new ExpenseWithinBudget(departmentId: $department->id, amount: -5000); // -50.00

    $fails = [];
    $rule->validate('amount', -5000, function ($message) use (&$fails): void {
        $fails[] = $message;
    });
...
test('department not found fails validation', function (): void {
    $rule = new ExpenseWithinBudget(departmentId: 9999, amount: 50000); // Non-existent department

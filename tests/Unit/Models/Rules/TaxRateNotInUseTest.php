...
test('tax rate without line items passes validation', function () {
    $taxRate = TaxRate::factory()->create();
    $rule = new TaxRateNotInUse();

    $fails = [];
    $rule->validate('tax_rate_id', $taxRate->id, function ($message) use (&$fails) {
        $fails[] = $message;
    });
...
test('tax rate with line items fails validation', function () {
    $taxRate = TaxRate::factory()->create();
...
test('tax rate with multiple line items fails validation with correct count', function () {
    $taxRate = TaxRate::factory()->create();
